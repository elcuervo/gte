...
RSpec.describe 'Binary tensor path', if: GTE_E5_AVAILABLE do
  ...
  it 'Pool#embed_binary returns row 0 as bytes' do
    bytes = pool.embed_binary(text)
    arr = pool.embed(text).row(0)
  ...
  it 'binary path retained-object footprint matches single String, not Array+slots' do
    ...
    bins = Array.new(50) { pool.embed_binary(text) }
